...
const TRANSLATIONS = {
  tr: {
    ...
    privacyText:
      "StyleSync olarak gizliliğinize önem veriyoruz. Bu uygulama, gardırop verilerinizi yerel olarak cihazınızda (LocalStorage) saklar. Fotoğraflarınız ve kişisel verileriniz harici sunuculara yüklenmez.",
    securityTitle: "Veri Güvenliği",
    localData: "Yerel Depolama",
    encrypted: "Güvenli İşlem",
    helpTitle: "Yardım Merkezi",
    faq1: "Nasıl kıyafet eklerim?",
    faq1A:
      'Ana sayfadaki veya gardırop sekmesindeki "Ekle" butonuna tıklayarak fotoğraf çekebilir veya galeriden yükleyebilirsiniz.',
    faq2: "Kombinler nasıl oluşturuluyor?",
    faq2A:
      "Yapay zeka algoritması, seçtiğiniz tarza ve renklere uygun parçaları gardırobunuzdan eşleştirir.",
  ...
  en: {
    ...
    privacyText:
      "At StyleSync, we value your privacy. This app stores your wardrobe data locally on your device (LocalStorage). Your photos and personal data are not uploaded to external servers.",
    ...
    faq1A:
      'Click the "Add" button on the home page or wardrobe tab to take a photo or upload from gallery.',
    faq2: "How are outfits created?",
    faq2A:
      "The AI algorithm matches items from your wardrobe that fit your selected style and colors.",
      ...
      <div className="flex-1 flex flex-col justify-center items-center text-center space-y-8">
        ...
        <div className="space-y-2 animate-in slide-in-from-bottom-4 duration-700">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
            {isLogin
              ? TRANSLATIONS[lang].loginTitle
              : TRANSLATIONS[lang].signupTitle}
          </h1>
          <p className="text-gray-500 dark:text-gray-400">
            {isLogin
              ? TRANSLATIONS[lang].loginSubtitle
              : TRANSLATIONS[lang].signupSubtitle}
          </p>
        </div>
        ...
        <div className="w-full space-y-3 max-w-sm animate-in slide-in-from-bottom-8 duration-700 delay-100">
          ...
          >
            <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
              <path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.74s2.57-.99 3.87-.84c1.61.12 2.87.69 3.6 1.74-3.2 1.6-2.67 6.08.54 7.64-.69 1.49-1.58 2.92-3.09 3.69zm-4.76-17.7c-1.35-.06-2.6 1.22-2.3 2.65 1.35.13 2.53-1.22 2.3-2.65z" />
            </svg>
        ...
        <div className="w-full max-w-sm space-y-3 animate-in slide-in-from-bottom-8 duration-700 delay-300">
          ...
          <button
            ...
            className="w-full bg-purple-600 text-white p-4 rounded-xl font-bold shadow-lg shadow-purple-200 dark:shadow-purple-900/30 hover:bg-purple-700 transition-all active:scale-95"
          >
...
const PrivacyPage = ({ lang, onBack }) => (
  <div className="fixed inset-0 bg-gray-50 dark:bg-gray-950 z-50 overflow-y-auto animate-in slide-in-from-right duration-300">
    <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 p-4 flex items-center gap-3 z-10">
      <button
        onClick={onBack}
        className="p-2 -ml-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-200 transition-colors"
      >
        <ChevronLeft size={24} />
      </button>
    ...
    <div className="p-6">
      <div className="bg-white dark:bg-gray-900 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 text-gray-600 dark:text-gray-300 leading-relaxed">
        ...
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4 text-center">
          {TRANSLATIONS[lang].securityTitle}
        </h3>
...
// --- SETTINGS VIEW (AYARLAR EKRANI) ---
const SettingsView = ({ lang, setLang, theme, setTheme, onBack, onLogout }) => {
  const [subPage, setSubPage] = useState(null);

  if (subPage === "privacy")
    return <PrivacyPage lang={lang} onBack={() => setSubPage(null)} />;
  if (subPage === "help")
    return <HelpPage lang={lang} onBack={() => setSubPage(null)} />;
        ...
        <section className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-800 divide-y divide-gray-100 dark:divide-gray-800">
          ...
          >
            <span className="font-medium flex items-center gap-2">
              <Shield size={18} /> {TRANSLATIONS[lang].privacy}
            </span>
          ...
          >
            <span className="font-medium flex items-center gap-2">
              <HelpCircle size={18} /> {TRANSLATIONS[lang].help}
            </span>
            <ChevronLeft size={18} className="rotate-180 text-gray-400" />
          </button>
          <button
          ...
          >
            <span className="font-medium flex items-center gap-2">
              <LogOut size={18} /> {TRANSLATIONS[lang].logout}
            </span>
          </button>
        </section>
      </div>
...
// 1. KIYAFET EKLEME MODALI
const AddItemModal = ({ isOpen, onClose, onAdd, lang }) => {
  ...
  const categories = [
    { id: "ust", label: "Üst Giyim", icon: Shirt },
    { id: "alt", label: "Alt Giyim", icon: Scissors },
    { id: "ayakkabi", label: "Ayakkabı", icon: Footprints },
    { id: "dis_giyim", label: "Dış Giyim", icon: Cloud },
    { id: "saat", label: "Saat", icon: Watch },
    { id: "gozluk", label: "Gözlük", icon: Glasses },
    { id: "kolye", label: "Kolye", icon: Gem },
    { id: "kupe", label: "Küpe", icon: Gem },
    { id: "bileklik", label: "Bileklik", icon: Gem },
  ];
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setAiResults(null);
      setProcessingStep("bg-remove");

      reader.onloadend = () => {
        setTimeout(() => {
          setProcessingStep("scan");
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              const randomSeason =
                SEASONS_DATA[Math.floor(Math.random() * SEASONS_DATA.length)];
              ...
              setFormData((prev) => ({
                ...
                fabric: randomFabric,
                fit: randomFit,
                name: `${randomColor.name} ${randomFabric} Parça`,
              }));
  ...
  const handleSubmit = () => {
    if (!preview || !formData.name) return;
    onAdd({
      id: Date.now(),
      imageUrl: preview,
      ...formData,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-900 w-full max-w-md h-[95vh] sm:h-auto sm:rounded-2xl rounded-t-2xl p-6 flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2">
            <Wand2 size={20} className="text-purple-600" />
            {TRANSLATIONS[lang].aiAnalyze}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2">
          <div
            ...
            className={`w-full aspect-video rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group ${
              preview
                ? "border-purple-500 bg-white dark:bg-gray-800 shadow-inner"
                : "border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-500 bg-gray-50 dark:bg-gray-800"
            }`}
          >
            {isProcessing ? (
              <div className="absolute inset-0 bg-white/95 dark:bg-gray-900/95 flex flex-col items-center justify-center z-20 gap-4">
                <div className="relative">
                  <Shirt
                    size={48}
                    className="text-gray-200 dark:text-gray-700"
                  />
                  <div className="absolute inset-0 border-b-2 border-purple-500 animate-[scan_1.5s_ease-in-out_infinite] bg-gradient-to-b from-transparent to-purple-500/20 h-full w-full origin-top"></div>
                </div>

                <div className="text-center">
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-200 animate-pulse">
                    {processingStep === "bg-remove" &&
                      "Arka Plan İzole Ediliyor..."}
                    {processingStep === "scan" && "Doku ve Desen Taranıyor..."}
                    {processingStep === "analyze" &&
                      "Renk ve Stil Eşleştiriliyor..."}
                  </p>
                </div>
              </div>
            ) : preview ? (
              <>
                <div className="relative w-full h-full flex items-center justify-center bg-white dark:bg-gray-800">
                  <img
                    src={preview}
                    alt="Preview"
                    className="max-h-full max-w-full object-contain drop-shadow-2xl scale-90 hover:scale-95 transition-transform duration-500"
                  />
                  {aiResults && (
                    <div className="absolute top-2 left-2 flex flex-col gap-1 items-start">
                      <div className="bg-black/70 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1 shadow-sm animate-in slide-in-from-left fade-in duration-500">
                        <Sparkles size={8} className="text-yellow-400" />%
                        {aiResults.confidence} Güven Skoru
                      </div>
                    </div>
                  )}
                </div>
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  <span className="text-white text-sm font-medium flex items-center gap-2">
                    <Camera size={16} /> Yeniden Çek
                  </span>
                </div>
              </>
            ) : (
              <>
                <div className="w-16 h-16 bg-purple-50 dark:bg-gray-700 text-purple-600 dark:text-purple-400 rounded-full flex items-center justify-center mb-3 group-hover:scale-110 transition-transform shadow-sm border border-purple-100 dark:border-gray-600">
                  <ScanLine size={32} />
                </div>
                <span className="text-gray-700 dark:text-gray-300 font-bold text-sm">
                  {TRANSLATIONS[lang].photoUpload}
                </span>
                <span className="text-[10px] text-gray-400 dark:text-gray-500 mt-1">
                  {TRANSLATIONS[lang].autoRemove}
                </span>
              </>
            )}
            <input
          ...
          <div className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-1.5 uppercase tracking-wider ml-1">
                {TRANSLATIONS[lang].itemName}
              </label>
              <input
                type="text"
                placeholder="Örn: Gümüş Saat"
                className="w-full p-3.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all font-medium text-gray-800 dark:text-white"
                value={formData.name}
                onChange={(e) =>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-wider ml-1">
              ...
              <div className="grid grid-cols-3 gap-2">
                {categories.map((cat) => (
                  ...
                  >
                    <cat.icon
                      size={20}
                      strokeWidth={formData.type === cat.id ? 2.5 : 2}
                    />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-wider ml-1 flex items-center gap-1">
                  <Palette size={12} /> {TRANSLATIONS[lang].color} (AI)
                </label>
                <div className="flex flex-wrap gap-2 bg-gray-50 dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
                  {COLORS.map((c) => (
                    <button
                      key={c.hex}
                      onClick={() => setFormData({ ...formData, color: c.hex })}
                      className={`w-6 h-6 rounded-full border border-gray-200 dark:border-gray-600 transition-transform ${
                        formData.color === c.hex
                          ? "scale-125 ring-2 ring-purple-500 ring-offset-2"
                          : ""
                      }`}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-400 dark:text-gray-500 mb-2 uppercase tracking-wider ml-1 flex items-center gap-1">
                  <User size={12} /> {TRANSLATIONS[lang].style} (AI)
                </label>
                <select
                  className="w-full p-3 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm font-medium appearance-none text-gray-800 dark:text-white"
        ...
        <div className="pt-4 mt-2 border-t border-gray-100 dark:border-gray-800">
          <button
            onClick={handleSubmit}
            disabled={!preview || !formData.name || isProcessing}
            className={`w-full py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 transition-all shadow-lg ${
              preview && formData.name && !isProcessing
                ? "bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 transform active:scale-95 shadow-purple-200 dark:shadow-none"
  ...
  const filteredWardrobe =
    activeCat === "Tümü"
      ...
      : wardrobe.filter((item) => {
          ...
          if (activeCat === "Aksesuar")
            return ["saat", "gozluk", "kolye", "kupe", "bileklik"].includes(
              type
            );
          return false;
        });
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
            {TRANSLATIONS[lang].myWardrobe}
          </h2>
          <p className="text-xs text-gray-500 dark:text-gray-400">
            {wardrobe.length} {TRANSLATIONS[lang].itemsCount}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 animate-in slide-in-from-bottom-4 duration-500">
        {filteredWardrobe.length === 0 ? (
          ...
          filteredWardrobe.map((item) => (
            <div
              key={item.id}
              className="bg-white dark:bg-gray-900 p-3 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col items-center gap-3 relative group hover:shadow-md transition-all"
            >
              {/* SİLME BUTONU - GÜNCELLENMİŞ: Her zaman görünür ve erişilebilir */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                ...
                className="absolute top-2 right-2 z-50 bg-white shadow-lg border border-gray-200 rounded-full p-2 text-red-500 hover:bg-red-50 active:scale-95 transition-all"
                title="Sil"
              >
              ...
              <div className="text-left w-full px-1">
                <h3 className="text-sm font-bold truncate text-gray-800 dark:text-gray-200">
                  {item.name}
                </h3>
...
// 3. ATÖLYE GÖRÜNÜMÜ (YENİ 4 AŞAMALI AKORDEON - GELİŞTİRİLMİŞ)
const StudioView = ({ wardrobe, lang }) => {
  ...
  const generateOutfit = () => {
    ...
    setTimeout(() => {
      ...
      const jewelry = wardrobe.filter((i) =>
        ["kolye", "kupe", "bileklik"].includes(i.type)
      );
      ...
      <div className="bg-white dark:bg-gray-900 sticky top-14 z-30 shadow-sm border-b border-gray-100 dark:border-gray-800 p-4 space-y-3">
          ...
          >
            ...
            <span className="text-xs text-purple-600 dark:text-purple-400 font-bold">
              {selections.colors.length > 0
                ? `${selections.colors.length} Seçildi`
                : ""}
            </span>
          </button>
          {activeStep === 3 && (
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {COLORS.map((c) => {
                  ...
                  return (
                    ...
                    >
                      {isSelected && (
                        <Check size={16} color={c.text} strokeWidth={3} />
                      )}
                    </button>
                  );
                })}
        ...
        >
          <Sparkles
            ...
            className={
              isGenerating
                ? "animate-spin text-yellow-400 dark:text-purple-600"
                : "text-yellow-400 dark:text-purple-600"
            }
          />
          {isGenerating
            ? TRANSLATIONS[lang].preparing
            : TRANSLATIONS[lang].combineAI}
        </button>
      </div>

      {/* --- SONUÇ ALANI (UZATILMIŞ DİKEY TASARIM) --- */}
      <div className="flex-1 p-4 flex flex-col items-center">
        {/* Min-height artırıldı (800px) */}
          ...
          <div className="relative z-10 flex-1 p-4 flex flex-col h-full">
            {!currentOutfit.top && !currentOutfit.bottom ? (
              ...
              <div className="flex-1 flex gap-4 h-full animate-in zoom-in duration-500">
                ...
                {/* SAĞ KOLON: AKSESUARLAR & AYAKKABI */}
                <div className="w-1/3 flex flex-col gap-4">
                  ...
                  <div className="h-24 bg-gray-50 dark:bg-gray-800/50 rounded-2xl flex items-center justify-center relative border border-gray-100 dark:border-gray-700 border-dashed overflow-hidden">
                    {currentOutfit.glasses ? (
                      <div className="absolute inset-0 p-2 flex items-center justify-center">
                        <SafeImage
                          src={currentOutfit.glasses.imageUrl}
                  ...
                  <div className="h-24 bg-gray-50 dark:bg-gray-800/50 rounded-2xl flex items-center justify-center relative border border-gray-100 dark:border-gray-700 border-dashed overflow-hidden">
                    {currentOutfit.watch ? (
                      <div className="absolute inset-0 p-2 flex items-center justify-center">
                        <SafeImage
                          src={currentOutfit.watch.imageUrl}
                          className="w-full h-full object-contain"
                        />
                      </div>
                    ) : (
                  ...
                  {/* Diğer Takı (Kolye/Küpe) */}
                  <div className="h-24 bg-gray-50 dark:bg-gray-800/50 rounded-2xl flex items-center justify-center relative border border-gray-100 dark:border-gray-700 border-dashed overflow-hidden">
                    {currentOutfit.jewelry ? (
                      <div className="absolute inset-0 p-2 flex items-center justify-center">
                        <SafeImage
                          src={currentOutfit.jewelry.imageUrl}
                          className="w-full h-full object-contain"
                        />
                      </div>
                    ) : (
...
// --- PROFILE VIEW (PROFİL EKRANI) ---
const ProfileView = ({ onOpenSettings, lang, user, onChangePhoto }) => {
  const fileInputRef = useRef(null);
  ...
  return (
    <div className="p-4 flex flex-col items-center justify-center min-h-[60vh] text-center bg-white dark:bg-gray-950 transition-colors duration-300">
      <div className="relative mb-6">
        <div
          onClick={() => fileInputRef.current?.click()}
          className="w-28 h-28 rounded-full border-4 border-white dark:border-gray-800 shadow-2xl overflow-hidden cursor-pointer group relative"
        >
          {user?.photo ? (
            <img
              src={user.photo}
      ...
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
        {user?.name || TRANSLATIONS[lang].guestUser}
      </h2>
      <p className="text-gray-500 dark:text-gray-400 text-sm mb-8 flex items-center gap-2 justify-center">
        <span className="w-2 h-2 rounded-full bg-green-500"></span>
        {user?.email || TRANSLATIONS[lang].prototypeMode}
      </p>

      <div className="w-full max-w-xs space-y-3">
        <button
          disabled
          className="w-full bg-gray-800 dark:bg-gray-700 text-white/50 dark:text-white/30 py-4 rounded-2xl font-bold shadow-none cursor-not-allowed flex items-center justify-center gap-2 relative group"
        >
          <ScanLine size={20} />
          {TRANSLATIONS[lang].bodyScan}

          <div className="absolute -top-3 -right-2 bg-purple-500 text-white text-[10px] px-2 py-1 rounded-full shadow-lg flex items-center gap-1 animate-bounce">
            <Lock size={10} /> {TRANSLATIONS[lang].soon}
          </div>
        </button>

        <button
          onClick={onOpenSettings}
          className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 py-4 rounded-2xl font-bold hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors flex items-center justify-center gap-2"
        >
          <Settings size={20} />
          {TRANSLATIONS[lang].settings}
        </button>
...
const App = () => {
  ...
  const handleLogin = (provider, data) => {
    ...
    const mockUser = {
      ...
      photo: null, // Başlangıçta fotoğraf yok
    };
    setUser(mockUser);
  };
  ...
  const handleLogout = () => {
    ...
    setActiveTab("wardrobe"); // Reset tab
  };
  ...
  if (!user) {
    return (
      <div
        className={`max-w-md mx-auto bg-white dark:bg-gray-950 min-h-screen font-sans ${
          theme === "dark" ? "dark" : ""
        }`}
      >
      ...
      {showSettings ? (
        ...
        <>
          ...
          <main className="h-full animate-in fade-in duration-500">
            ...
            {activeTab === "studio" && (
              <StudioView wardrobe={wardrobe} lang={lang} />
            )}
            {activeTab === "profile" && (
              <ProfileView
